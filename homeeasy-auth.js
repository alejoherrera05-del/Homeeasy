...
(function (global) {
    ...
    const config = Object.freeze({
        ...
        defaultPersistence: VALID_PERSISTENCE.has(rawConfig.defaultPersistence)
            ...
            : 'session',
        activationPath: String(rawConfig.activationPath || 'activar-cuenta.html').trim() || 'activar-cuenta.html'
    });
    ...
    function friendlyMessage(code) {
        ...
        const messages = {
            ...
            EMAIL_EXISTS: 'Ya existe una cuenta con este correo. Usa tu contraseña actual para activar la invitación.',
            INVITE_ACTIVATION_REQUIRED: 'Esta cuenta tiene una invitación pendiente. Ábrela desde el enlace de activación.',
            INVALID_INVITATION: 'La invitación no es válida o venció.',
    ...
    async function openAppSession(options) {
        ...
        const response = await requestBackend({
            ...
            activationToken: String(opts.activationToken || ''),
            meta: opts.meta || {}
        });
    ...
    async function validateInvitation(email, inviteToken) {
        ensureConfigured();
        const cleanEmail = normalizeEmail(email);
        const token = String(inviteToken || '').trim();
        if (!cleanEmail || !token) throw new HomeEasyAuthError('INVALID_INVITATION', friendlyMessage('INVALID_INVITATION'));
        const response = await requestBackend({
            tipo: 'AUTH_VALIDAR_INVITACION',
            email: cleanEmail,
            inviteToken: token,
            meta: buildMeta({ pagina: config.activationPath })
        }, { timeoutMs: 15000 });
        if (!response || response.status !== 'success' || response.valido !== true) {
            throw new HomeEasyAuthError(response && response.code || 'INVALID_INVITATION', response && response.msg || friendlyMessage('INVALID_INVITATION'), response);
        }
        return Object.freeze({
            valido: true,
            usuario: response.usuario || {},
            nombre: response.usuario && response.usuario.nombre || '',
            email: response.usuario && response.usuario.email || cleanEmail,
            rol: response.usuario && response.usuario.rol || '',
            expiraEn: response.expiraEn || ''
        });
    }

    async function activateInvitation(email, password, inviteToken, options) {
        ensureConfigured();
        const cleanEmail = normalizeEmail(email);
        const cleanPassword = String(password || '');
        const token = String(inviteToken || '').trim();
        if (cleanPassword.length < 8) throw new HomeEasyAuthError('WEAK_PASSWORD', 'La contraseña debe tener al menos 8 caracteres.');
        await validateInvitation(cleanEmail, token);

        let payload;
        try {
            payload = await requestFirebase(firebaseEndpoint('signUp'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: cleanEmail, password: cleanPassword, returnSecureToken: true })
            });
        } catch (error) {
            // Si la persona ya tenía una cuenta Firebase, la contraseña escrita debe
            // corresponder a esa cuenta. No se vincula jamás un correo por coincidencia.
            if (error && error.code === 'EMAIL_EXISTS') {
                try {
                    payload = await requestFirebase(firebaseEndpoint('signInWithPassword'), {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ email: cleanEmail, password: cleanPassword, returnSecureToken: true })
                    });
                } catch (signInError) {
                    throw new HomeEasyAuthError('EMAIL_EXISTS', 'Ese correo ya tiene una cuenta. Escribe su contraseña actual para completar la activación.', signInError);
                }
            } else {
                throw error;
            }
        ...
        const session = sessionFromSignIn(payload, options && options.remember === true ? 'local' : 'session', {
            appSessionToken: '', appSessionExpiresAt: '', appSessionValidatedAt: 0, profile: {}, permissions: []
        });
        emitAuthChange('firebase-signed-in', sessionToUser(session));
        try {
            const activation = await requestBackend({
                tipo: 'AUTH_ACTIVAR_INVITACION',
                firebaseIdToken: session.idToken,
                inviteToken: token,
                meta: options && options.meta ? options.meta : buildMeta({ pagina: config.activationPath })
            }, { timeoutMs: 20000 });
            if (!activation || activation.status !== 'success' || activation.valido !== true) {
                throw new HomeEasyAuthError(activation && activation.code || 'INVITE_ACTIVATION_FAILED', activation && activation.msg || 'No se pudo activar la invitación.', activation);
            }
            await openAppSession({ meta: options && options.meta ? options.meta : buildMeta({ pagina: config.activationPath }) });
        } catch (error) {
            clearStoredSessions();
            emitAuthChange('session-rejected', null);
            throw error;
    ...
    function touchPresence() {
        const current = readStoredSession();
        if (!current || !current.appSessionToken || !isConfigured()) return Promise.resolve(false);
        return requestBackend({
            tipo: 'AUTH_PRESENCIA',
            appSessionToken: current.appSessionToken,
            meta: buildMeta()
        }, { timeoutMs: 8000 }).then(() => true).catch(() => false);
    }
    ...
    global.setTimeout(() => { if (getAppSessionToken()) startPresenceHeartbeat(); }, 1200);

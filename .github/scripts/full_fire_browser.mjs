...
function mockData(req){
  ...
  if(tipo==='AUTH_VALIDAR_SESION') return {status:'success',valido:true,perfil:{uid:'audit-owner',nombre:'Audit Owner',email:'audit@example.com',rol:'PROPIETARIO',estado:'ACTIVO'},permisos:permissions,expiresAt:new Date(Date.now()+4*3600_000).toISOString()};
...
await check('temporary backend outage behavior on Index',async()=>{const {context,page}=await makePage({stale:true,networkFail:true}); let navErr=''; try{await page.goto(base+'index.html',{waitUntil:'domcontentloaded',timeout:5000});}catch(e){navErr=e.message} await page.waitForTimeout(1800); const state={url:page.url(),nav:await page.locator('.nav-groups').count(),pending:await page.evaluate(()=>document.documentElement.classList.contains('homeeasy-auth-pending')).catch(()=>null)}; notes.push(`Index outage: ${JSON.stringify(state)}${navErr?' nav='+navErr:''}`); if(state.url.includes('login.html')) throw Error('redirected to login on transient network failure'); if(!state.nav) throw Error('Index became unavailable on transient network failure'); await context.close();});

await check('temporary backend outage behavior on module',async()=>{const {context,page}=await makePage({stale:true,networkFail:true}); await page.goto(base+'clientes.html',{waitUntil:'domcontentloaded',timeout:5000}).catch(()=>{}); await page.waitForTimeout(1800); const state={url:page.url(),guard:await page.evaluate(()=>window.HomeEasyPageGuard?window.HomeEasyPageGuard.getStatus():'missing').catch(()=> 'gone')}; notes.push(`Module outage: ${JSON.stringify(state)}`); if(state.url.includes('login.html')) throw Error('redirected to login on transient network failure'); if(state.guard!=='authorized') throw Error(`guard became ${state.guard}`); await context.close();});
...
await check('AR mobile has no horizontal overflow',async()=>{const context=await browser.newContext({viewport:{width:390,height:844}}); const page=await context.newPage(); await page.goto(base+'ar-homeeasy-v3.html',{waitUntil:'domcontentloaded'}); await page.waitForTimeout(1600); const o=await page.evaluate(()=>document.documentElement.scrollWidth>document.documentElement.clientWidth+2); if(o) throw Error('horizontal overflow'); await context.close();});

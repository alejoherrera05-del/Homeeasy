...
await check('definitive session revocation still forces login',async()=>{const {context,page}=await makePage({stale:true,backendMode:'revoked'}); await page.goto(base+'clientes.html',{waitUntil:'domcontentloaded'}); await page.waitForURL('**/login.html?**',{timeout:3500}); const stored=await page.evaluate(()=>sessionStorage.getItem('HOMEEASY_AUTH_SESSION_V1')); if(stored) throw Error('revoked session still stored'); await context.close();});
